...
class DatabaseEntriesRepository implements Contract, ClearableRepository, PrunableRepository, TerminableRepository
{
    ...
    public function find($id): EntryResult
    {
        ...
        $tags = $this->table($this->telescope_entries_tags)
                        ->where('entry_uuid', $id)
                        ->pluck('tag')
                        ->all();
    ...
    {
        return $this->table($this->telescope_entries)
    ...
    public function store(Collection $entries)
    {
        ...
        $table = $this->table($this->telescope_entries);
    ...
    protected function storeExceptions(Collection $exceptions)
    {
        $exceptions->chunk($this->chunkSize)->each(function ($chunked) {
            $this->table($this->telescope_entries)->insert($chunked->map(function ($exception) {
                $occurrences = $this->countExceptionOccurences($exception);

                $this->table($this->telescope_entries)
                        ->where('type', EntryType::EXCEPTION)
                        ->where('family_hash', $exception->familyHash())
    ...
    protected function storeTags(Collection $results)
    {
        $results->chunk($this->chunkSize)->each(function ($chunked) {
            $this->table($this->telescope_entries_tags)->insert($chunked->flatMap(function ($tags, $uuid) {
    ...
    public function update(Collection $updates)
    {
        foreach ($updates as $update) {
            $entry = $this->table($this->telescope_entries)
    ...
    protected function updateTags($entry)
    {
        if (! empty($entry->tagsChanges['added'])) {
            $this->table($this->telescope_entries_tags)->insert(
        ...
        collect($entry->tagsChanges['removed'])->each(function ($tag) use ($entry) {
            $this->table($this->telescope_entries_tags)->where([
                'entry_uuid' => $entry->uuid,
                'tag' => $tag,
            ])->delete();
        });
    ...
    {
        return $this->table($this->telescope_monitoring)->pluck('tag')->all();
    }
    ...
    public function monitor(array $tags)
    {
        ...
        $this->table($this->telescope_monitoring)
                    ->insert(collect($tags)
    ...
    {
        $this->table($this->telescope_monitoring)->whereIn('tag', $tags)->delete();
    }
    ...
    {
        $query = $this->table($this->telescope_entries)
    ...
    {
        $this->table($this->telescope_entries)->delete();
        $this->table($this->telescope_monitoring)->delete();
    }
