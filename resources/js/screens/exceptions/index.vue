    ...
    <index-screen title="Exceptions" resource="exceptions">
        ...
        <template slot="row" slot-scope="slotProps">
            <td :title="slotProps.entry.content.class" v-if="!$route.query.family_hash">
                {{truncate(slotProps.entry.content.class, 80)}}<br>
                <small class="text-muted">{{truncate(slotProps.entry.content.message, 100)}}</small>
            </td>

            <td class="table-fit align-middle" v-if="!$route.query.family_hash && !$route.query.tag">
            ...
            <td class="table-fit align-middle">{{timeAgo(slotProps.entry.created_at)}}</td>
